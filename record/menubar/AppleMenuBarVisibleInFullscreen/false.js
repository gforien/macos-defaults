...
module.exports = {
  run: async (outputPath) => {
    console.log(
      '> Recording menu bar clock AppleMenuBarVisibleInFullscreen with param set to false',
    )
    ...
    const { stderr: setEnvError } = await exec(
      `defaults write NSGlobalDomain AppleMenuBarVisibleInFullscreen -bool false && killall SystemUIServer && sleep 10`,
    )

    if (setEnvError) {
      console.error(
        'An error occured while setting up the menu bar clock AppleMenuBarVisibleInFullscreen command',
      )
      logRollbackInfo()
    ...
    const { stderr: deleteEnvError } = await exec(
      'defaults delete NSGlobalDomain AppleMenuBarVisibleInFullscreen && killall SystemUIServer && sleep 10',
    )
    if (deleteEnvError) {
      console.error(
        'An error occured while cleaning the menu bar clock AppleMenuBarVisibleInFullscreen environment',
      )
      logRollbackInfo()
...
function logRollbackInfo() {
  ...
  console.info(
    'defaults delete NSGlobalDomain AppleMenuBarVisibleInFullscreen && killall SystemUIServer',
  )
}
